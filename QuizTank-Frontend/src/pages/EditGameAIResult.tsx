...
import { Sparkles, Save, Upload, Plus, Trash2, X, Check, Lock, ChevronUp, ChevronDown, CheckCircle, RefreshCw, XCircle } from "lucide-react";
...
export default function EditGameAIResult() {
    const { id } = useParams();
    const navigate = useNavigate();
    const location = useLocation();
    const { user } = useAuth();
    const { generatedData, originalPrompt } = location.state || {};
    ...
    // Fetch game data on mount and merge AI data
    useEffect(() => {
        const fetchGame = async () => {
            if (!id || !generatedData || Object.keys(generatedData).length === 0) return;
            setIsLoading(true);
            try {
                ...
                // Initialize media maps from original game (will be cleared if overridden by AI)
                ...
                let initialFormData = {
                    name: data.name || "",
                    visibility: getVisibility(data.visibility) as 'public' | 'private' | 'locked' | 'unlisted',
                ...
                if (generatedData) {
                    // Update fields that AI provides
                    initialFormData.name = generatedData.name || initialFormData.name;
                    initialFormData.category = generatedData.category || initialFormData.category;
                    initialFormData.language = generatedData.language || initialFormData.language;
                    initialFormData.tags = generatedData.tags || initialFormData.tags;
                    initialFormData.description = generatedData.description || initialFormData.description;

                    // Replace Questions
                    if (generatedData.questions && Array.isArray(generatedData.questions)) {
                        initialFormData.questions = generatedData.questions.map((q: any, idx: number) => {
                            const qId = `q${idx + 1}`;
                            let type: Question['type'] = 'single';
                            if (q.type === 2) type = 'multiple';
                            if (q.type === 3) type = 'fill';

                            const choices = (q.choices || []).map((c: any, cIdx: number) => ({
                                id: `${qId}-c${cIdx + 1}`,
                                text: c.content || c.text || "",
                                isCorrect: c.correct === 1 || c.isCorrect
                            }));
                        ...
                        // Reset question media since IDs match new questions
                        /* We don't clear qMediaMap variable but we don't use it for new questions. 
                           Wait, we called setQuestionMedia(qMediaMap) later? No, we haven't set it yet.
                           We should NOT use qMediaMap for these new questions.
                        */
                        // So effectively:
                        // qMediaMap = {}; // Reset local map
                        // But TypeScript might complain if const.
                    }

                    // Replace Knowledges
                    if (generatedData.knowledges && Array.isArray(generatedData.knowledges)) {
                        initialFormData.knowledges = generatedData.knowledges.map((k: any, idx: number) => ({
                            id: `k${idx + 1}`,
                            content: k.content,
                            mediaUrl: ""
                        }));
                    ...
                    // Note: We are discarding old media mappings effectively if we replace questions/knowledges
                    // If we didn't replace them (e.g. partial update?), we might keep them. 
                    // But AI returns full lists.

                    // Actually set the media states
    ...
    const handleSave = async (publish = false) => {
        ...
        try {
            const payload = {
                ...
                ai_generated: generatedData ? 1 : undefined
            };

            await gameRoomService.updateGame(id, payload);
            toast.success("Game updated with AI changes successfully!");
    ...
    if ((!generatedData || Object.keys(generatedData).length === 0) && id) {
        return <Navigate to={`/games/edit/${id}`} replace />;
    }

    return (
        <PageLayout title="AI Generated Review">
            ...
            <div className="sticky bottom-0 bg-white py-6 px-4 mt-8">
                <div className="flex flex-col sm:flex-row justify-end gap-3">
                    ...
                    <Button
                        variant="outline"
                        onClick={() => navigate(`/games/edit-ai/${id}`, { state: { initialPrompt: originalPrompt } })}
